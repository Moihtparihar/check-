...
import { HypERC20Collateral__factory, HypERC20__factory, HypERC4626Collateral__factory, HypERC4626OwnerCollateral__factory, HypERC4626__factory, HypXERC20Lockbox__factory, HypXERC20__factory, IFiatToken__factory, ISafe__factory, IXERC20__factory, MovableCollateralRouter__factory, OpL1NativeTokenBridge__factory, OpL2NativeTokenBridge__factory, Ownable__factory, PackageVersioned__factory, ProxyAdmin__factory, TokenBridgeCctp__factory, TokenRouter__factory, } from '@hyperlane-xyz/core';
import { buildArtifact as coreBuildArtifact } from '@hyperlane-xyz/core/buildArtifact.js';
import { arrayToObject, assert, getLogLevel, isZeroishAddress, objFilter, objMap, promiseObjAll, rootLogger, } from '@hyperlane-xyz/utils';
import { DEFAULT_CONTRACT_READ_CONCURRENCY } from '../consts/concurrency.js';
import { isAddressActive } from '../contracts/contracts.js';
import { ContractVerifier } from '../deploy/verify/ContractVerifier.js';
import { ExplorerLicenseType, VerifyContractTypes, } from '../deploy/verify/types.js';
...
import { ContractVerificationStatus, HypTokenConfigSchema, OwnerStatus, XERC20Type, isMovableCollateralTokenConfig, } from './types.js';
...
export class EvmERC20WarpRouteReader extends EvmRouterReader {
    ...
    async deriveWarpRouteConfig(warpRouteAddress) {
        ...
        if (hasRebalancingInterface &&
            ...
            try {
                ...
                const allowedBridgesByDomain = await promiseObjAll(objMap(arrayToObject(knownDomains.map((domain) => domain.toString())), (domain) => movableToken.allowedBridges(domain)));
    ...
    async getContractVerificationStatus(chain, address) {
        ...
        if (contractType === VerifyContractTypes.Proxy) {
            contractVerificationStatus[VerifyContractTypes.Implementation] =
                await this.contractVerifier.getContractVerificationStatus(chain, await proxyImplementation(this.provider, address));
            // Derive ProxyAdmin status
            contractVerificationStatus[VerifyContractTypes.ProxyAdmin] =
                await this.contractVerifier.getContractVerificationStatus(chain, await proxyAdmin(this.provider, address));
        }
    ...
    async deriveHypCollateralCctpTokenConfig(hypToken) {
        ...
        const tokenBridge = TokenBridgeCctp__factory.connect(hypToken, this.provider);
    ...
    async deriveHypNativeTokenConfig(_address) {
    ...
    async fetchPackageVersion(address) {
        const contractWithVersion = PackageVersioned__factory.connect(address, this.provider);
        try {
    ...
    async fetchProxyAdminConfig(tokenAddress) {
        ...
        assert(!isZeroishAddress(proxyAdminAddress), `ProxyAdmin config for warp token at address "${tokenAddress}" can't be derived because it is not a proxy.`);
