...
import { ProtocolType, addressToBytes32, deepCopy, intersection, isAddressEvm, isCosmosIbcDenomAddress, isObjEmpty, objFilter, objMap, promiseObjAll, sortArraysInObject, transformObj, } from '@hyperlane-xyz/utils';
 ...
 */
const getGasConfig = (warpDeployConfig, chain) => warpDeployConfig[chain].gas?.toString() ||
...
export async function expandWarpDeployConfig(params) {
    const { multiProvider, warpDeployConfig, deployedRoutersAddresses, expandedOnChainWarpConfig, } = params;
    ...
    return promiseObjAll(objMap(warpDeployConfig, async (chain, config) => {
        ...
        const isEVMChain = multiProvider.getProtocol(chain) === ProtocolType.Ethereum;
        ...
        if (expandedOnChainWarpConfig?.[chain]?.ownerStatus &&
            multiProvider.getProtocol(chain) === ProtocolType.Ethereum) {
