...
describe('EvmTimelockDeployer', async () => {
    ...
    beforeEach(async () => {
        ...
        assert(signer.address === signerAddress, 'Expected signer.address to be equal signerAddress');
        assert(otherSigner.address === otherSignerAddress, 'Expected otherSigner.address address to be equal otherSignerAddress');
    });
    describe('basic config', async () => {
        const testCases = [
            ...
            {
                title: 'should deploy TimelockController with multiple proposers and executors',
                config: {
                    minimumDelay: 7200,
        ...
        for (const { title, config } of testCases) {
            it(title, async () => {
                const { TimelockController } = await deployer.deployContracts(TestChainName.test2, config);
                const timelockAddress = TimelockController.address;
                const timelock = TimelockController__factory.connect(timelockAddress, multiProvider.getProvider(TestChainName.test2));
    ...
    describe('multichain deployments', () => {
        ...
        it('should deploy different contracts for different chains', async () => {
            const { TimelockController: { address: address1 }, } = await deployer.deployContracts(TestChainName.test1, config);
            const { TimelockController: { address: address2 }, } = await deployer.deployContracts(TestChainName.test2, config);
    ...
    it('should allow anyone to execute a transaction if no one is set in the input config', async () => {
        const openExecutorRoleConfig = {
            minimumDelay: 0,
            proposers: [signerAddress],
            admin: signerAddress,
        };
        const { TimelockController } = await deployer.deployContracts(TestChainName.test4, openExecutorRoleConfig);
        const timelockAddress = TimelockController.address;
        ...
        const testTxData = TimelockController__factory.createInterface().encodeFunctionData('updateDelay', [updatedDelay]);
        const scheduleTx = await timelock.schedule(timelockAddress, 0, testTxData, EMPTY_BYTES_32, EMPTY_BYTES_32, 0);
        await scheduleTx.wait();
        const otherSignerTimelockInstance = TimelockController__factory.connect(timelockAddress, otherSigner);
        const executeTx = await otherSignerTimelockInstance.execute(timelockAddress, 0, testTxData, EMPTY_BYTES_32, EMPTY_BYTES_32);
    ...
    describe('canceller config', () => {
        it('should deploy with the correct canceller config', async () => {
            ...
            const cancellerConfig = {
                minimumDelay: 30,
                proposers: [proposer],
                executors: [signerAddress],
                cancellers: [randomAddress(), randomAddress()],
            };
            const { TimelockController } = await deployer.deployContracts(TestChainName.test4, cancellerConfig);
    ...
    describe('admin config', () => {
        ...
        const testCases = [
            {
                title: 'should deploy TimelockController with the timelock as the only admin when an admin address is not provided',
                config: {
                    minimumDelay: 1800,
                    proposers: [signerAddress],
                    executors: [signerAddress],
            ...
            {
                title: 'should not revoke the admin role from the deployer if it is the expected admin',
                config: {
                    minimumDelay: 30,
            ...
            {
                title: 'should set the expected admin from the config after applying the changes',
                config: {
                    minimumDelay: 30,
                    proposers: [proposer],
                    executors: [signerAddress],
        ...
        for (const { title, config } of testCases) {
            it(title, async () => {
                const { TimelockController } = await deployer.deployContracts(TestChainName.test3, config);
                const timelockAdminRoleHash = await TimelockController.TIMELOCK_ADMIN_ROLE();
                ...
                expect(await TimelockController.hasRole(timelockAdminRoleHash, expectedAdmin)).to.be.true;
            });
        }
    });
});
//# sourceMappingURL=EvmTimelockDeployer.hardhat-test.js.map