...
describe(EvmTimelockReader.name, () => {
    ...
    describe(`${EvmTimelockReader.name} (RPC)`, () => {
        ...
        describe(`${EvmTimelockReader.prototype.getPendingOperationIds.name}`, () => {
            ...
            for (const { title, timelockTxs, cancelledTxIndexes, executedTxIndexes, expectedPendingCount, } of pendingTestCases) {
                it(title, async () => {
                    const proposerTimelock = TimelockController__factory.connect(timelockAddress, proposer);
                    const executorTimelock = TimelockController__factory.connect(timelockAddress, executor);
                    const operationIds = [];
                    ...
                    const pendingIds = await timelockReader.getPendingOperationIds(operationIds);
