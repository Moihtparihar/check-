...
import { ProtocolType, assert, convertDecimals, objMap, } from '@hyperlane-xyz/utils';
...
export async function getCosmosChainGasPrice(chain, chainMetadataManager) {
    ...
    try {
        ...
        throw new Error(`No gas price found for Cosmos chain ${chain} in the registry or metadata`);
    }
...
function getTokenExchangeRate({ local, remote, tokenPrices, exchangeRateMarginPct, }) {
    ...
    assert(exchangeRate.isGreaterThan(0), 'Exchange rate must be greater than 0, possible loss of precision');
    return exchangeRate;
}
function getProtocolExchangeRate(localProtocolType, exchangeRate) {
