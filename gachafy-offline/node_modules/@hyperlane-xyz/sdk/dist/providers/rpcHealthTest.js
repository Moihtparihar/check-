...
export async function isStarknetJsProviderHealthy(provider, metadata) {
    const blockNumber = await provider.getBlockNumber();
    if (!blockNumber || blockNumber < 0)
        return false;
    rootLogger.debug(`Block number is okay for ${metadata.name}`);
...
export async function isRadixProviderHealthy(provider, metadata) {
    try {
        ...
        rootLogger.warn(`Radix gateway health check threw for ${metadata.name}`, err);
        return false;
    }
}
