...
export class EV5TimelockSubmitter {
    ...
    proposerSubmitter;
    timelockInstance;
    txSubmitterType = TxSubmitterType.TIMELOCK_CONTROLLER;
    constructor(config, multiProvider, proposerSubmitter, timelockInstance) {
        this.config = config;
    ...
    static async fromConfig(config, multiProvider, coreAddressesByChain) {
        ...
        const timelockInstance = TimelockController__factory.connect(config.timelockAddress, provider);
        const minDelay = (await timelockInstance.getMinDelay()).toBigInt();
        const delay = config.delay ?? minDelay;
        assert(delay >= minDelay, `Expected user supplied delay ${delay} to be greater or equal than the configured minDelay ${minDelay}`);
        ...
        }, multiProvider, proposerSubmitter, timelockInstance);
    }
    async submit(...txs) {
        if (txs.length === 0) {
            return [];
        ...
        if (transactionChains.size !== 1) {
            throw new Error('TimelockController transactions should have all the same destination chain');
        }
        ...
        const destinationChainDomainId = this.multiProvider.getDomainId(domainId ?? this.config.chain);
        const calldata = txs.map((transaction) => {
            assert(transaction.data, 'Invalid Transaction: data must be defined');
            assert(transaction.to, 'Invalid Transaction: target address must be defined');
            return {
                to: transaction.to,
        ...
        const [to, data, value] = calldata.reduce(([targets, data, values], item) => {
        ...
        const operationId = await this.timelockInstance.hashOperationBatch(to, value, data, this.config.predecessor, this.config.salt);
        const checkStatus = await this.timelockInstance.isOperation(operationId);
        assert(!checkStatus, `Operation with id "${operationId}" already exists. If this is a new operation with the same input as another one provide a salt to generate a different operation id or cancel the existing one if it is still pending.`);
        const [proposeCallData, executeCallData] = await Promise.all([
            this.timelockInstance.populateTransaction.scheduleBatch(to, value, data, this.config.predecessor, this.config.salt, this.config.delay),
            this.timelockInstance.populateTransaction.executeBatch(to, value, data, this.config.predecessor, this.config.salt),
        ]);
        ...
        // Appending the execute transaction so that it can be written to a file
        // by the caller
        return [...proposeTransactions, executeCallData];
    }
}
//# sourceMappingURL=EV5TimelockSubmitter.js.map