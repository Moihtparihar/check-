...
export class CosmosNativeHookReader {
    ...
    constructor(metadataManager, cosmosProviderOrSigner) {
    ...
    async deriveIgpConfig(address) {
        ...
        const { destination_gas_configs } = await this.cosmosProviderOrSigner.query.postDispatch.DestinationGasConfigs({
            id: igp.id,
        });
        const overhead = {};
        const oracleConfig = {};
        destination_gas_configs.forEach((gasConfig) => {
            const { name, nativeToken } = this.metadataManager.getChainMetadata(gasConfig.remote_domain);
    ...
    async deriveMerkleTreeConfig(address) {
        const { merkle_tree_hook } = await this.cosmosProviderOrSigner.query.postDispatch.MerkleTreeHook({
            id: address,
        });
        assert(merkle_tree_hook, `Merkle Tree Hook not found for address ${address}`);
        return {
            type: HookType.MERKLE_TREE,
            address: merkle_tree_hook.id,
        };
    }
    async deriveNoopConfig(address) {
        const { noop_hook } = await this.cosmosProviderOrSigner.query.postDispatch.NoopHook({
            id: address,
        });
    ...
    async isMerkleTreeHook(address) {
        try {
            const { merkle_tree_hook } = await this.cosmosProviderOrSigner.query.postDispatch.MerkleTreeHook({
                id: address,
            });
...
//# sourceMappingURL=CosmosNativeHookReader.js.map