...
y.styles = [
  ...
  css`
      ...
      button.gradient {
        ...
        background-image: url(${unsafeCSS(Se)});
        padding-right: 7px;
      }
...
var v = class extends LitElement {
  ...
  render() {
    const t = () => this.timestamp ? html`<div class="timestamp">${ue(this.timestamp)}</div>` : "", i = `${this.icon ? "30px" : ""} 1fr ${this.timestamp ? "42px" : ""}`;
...
Y.styles = [
  css`.link{color:var(--radix-link-color);font-weight:600;text-decoration:none;display:flex;gap:4px;align-items:center;font-size:14px;cursor:pointer}.icon-north-east-arrow::before{content:'';display:block;-webkit-mask-size:cover;mask-size:cover;background-color:var(--radix-card-text-dimmed-color);-webkit-mask-image:url('${unsafeCSS(ce)}');mask-image:url('${unsafeCSS(ce)}');width:16px;height:16px}`
];
xe([
  property({
    type: String
...
var Ne = (t) => t ? `${t.slice(0, 4)}...${t.slice(
  t.length - 6,
  t.length
)}` : "";
...
var N = class extends LitElement {
  ...
  render() {
    ...
    }}">${Ne(this.address)} <button aria-label="Copied!" class="${classMap({
...
var x = class extends LitElement {
  ...
  render() {
    const t = this.getIconFromStatus(), i = this.getStylingFromStatus(), r = {
      sendTransaction: {
        ...
        pendingCommit: "",
        timedOut: "",
        content: html`${this.renderHash()} ${this.status === "pending" ? html`<div class="request-content">Open your Radix Wallet app to review the transaction ${this.showCancel ? html`<div class="cancel" @click="${this.onCancel}">Cancel</div>` : html`<div class="cancel" @click="${this.onIgnore}">Ignore</div>`}</div>` : ""}`
      },
      preAuthorizationRequest: {
        ...
        content: html`${this.renderHash()} ${this.status === "pending" ? html`<div class="request-content">Open your Radix Wallet app to review the preauthorization ${this.showCancel ? html`<div class="cancel" @click="${this.onCancel}">Cancel</div>` : html`<div class="cancel" @click="${this.onIgnore}">Ignore</div>`}</div>` : this.status === A.pendingCommit ? html`<div class="request-content"><div class="cancel" @click="${this.onIgnore}">Ignore</div></div>` : ""}`
      },
      dataRequest: {
  ...
  getRequestContentTemplate(t) {
    return this.status === A.pending ? html`<div class="request-content">${t} ${this.showCancel ? html`<div class="cancel" @click="${this.onCancel}">Cancel</div>` : ""}</div>` : "";
  }
  hasErrorIcon(t) {
    return [
      A.cancelled,
  ...
  renderHash() {
    return this.hash ? html`<div class="transaction"><span class="text-dimmed">ID:</span><radix-link displayText="${Ne(this.hash)}" @click="${(t) => {
      t.preventDefault(), this.dispatchEvent(
        new CustomEvent("onLinkClick", {
          bubbles: true,
          composed: true,
...
var W = class extends LitElement {
  ...
  render() {
    return (this.requestItems || []).map(
      (t) => html`<radix-request-card type="${t.type}" status="${t.status}" id="${t.interactionId}" hash="${t.transactionIntentHash || ""}" ?showCancel="${t.showCancel}" timestamp="${t.createdAt}"></radix-request-card>`
    );
  }
};
W.styles = [
  R,
  css`:host{display:flex;flex-direction:column;gap:10px}`
];
be([
  property({ type: Array })
...
var D = class extends LitElement {
  ...
  render() {
    var t, i;
    return html`<div class="header">Sharing with ${this.dAppName || "dApp"}</div><div class="content"><radix-persona-card avatarUrl="${this.avatarUrl}" persona="${this.persona}" .personaData="${this.personaData}"></radix-persona-card><div>${(this.accounts || []).map(
      ({ label: r, address: o, appearanceId: e }) => html`<radix-account label="${r}" address="${o}" appearanceId="${e}"></radix-account>`
...
Q.styles = [
  css`:host{position:fixed;top:0;left:0;right:unset;height:100%;width:100%;padding:16px;box-sizing:border-box;backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);display:flex;align-items:center;justify-content:center;transition:opacity .2s;background:var(--radix-mask-background);z-index:2147483647}:host([isBranded]){align-items:flex-start;background:#000;background-image:url('${unsafeCSS(nt)}');background-size:cover}:host(.hide){opacity:0;pointer-events:none}:host(.show){opacity:1}`
];
he([
  property({
    type: Boolean,
...
var T = class extends LitElement {
  ...
  render() {
    return html`<div class="header">Connected to ${this.dAppName || "dApp"}</div><slot name="subheader"></slot>${this.loggedInTimestamp ? html`<div class="subheader">Since logged in: ${ue(this.loggedInTimestamp, ", ")}</div>` : ""}<div class="content"><radix-request-cards .requestItems="${this.requestItems}"></radix-request-cards></div>`;
  }
};
T.styles = [
  De,
  css`.subheader{color:var(--radix-card-text-dimmed-color);margin-top:-12px;margin-bottom:15px;text-align:center;font-size:12px}`
];
X([
  property({ type: Array })
...
var I = class extends LitElement {
  constructor() {
...
var B = class extends LitElement {
  ...
  render() {
    return html`<div class="radix-wallet-connector-card"><div class="radix-wallet-connector-card__logo"><img width="78" height="78" src="${Ae}" alt="Radix Wallet Connector Logo"></div><div class="radix-wallet-connector-card__header">${this.header}</div><slot></slot></div>`;
  }
};
B.styles = [
...
var L = class extends LitElement {
  ...
  render() {
    return html`<div class="connector-error">${this.isError ? html`<img src="${ut}" alt="Warning" width="33">` : ""}<h1 class="${classMap({ error: this.isError })}">${this.header}</h1>${this.subheader ? html`<h3>${this.subheader}</h3>` : ""}</div>`;
  }
};
L.styles = [
...
lit-html/development/directives/style-map.js:
   ...
   *)
*/
//# sourceMappingURL=connect-button-CHA436HC-DHF6JSYE.js.map
